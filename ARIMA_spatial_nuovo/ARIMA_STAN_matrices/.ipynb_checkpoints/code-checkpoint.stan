...
transformed data {
  ...
  int number_of_couples_of_stations = S*(S-1)/2;
  array[number_of_couples_of_stations] real distance_between_two_stations;

  index = 1;
  for (i in 1:S) {
    for (j in (i+1):S) {
      distance_between_two_stations[index] = distance(coord[i],coord[j]);
      index += 1;
    }
  }
  
//  real rho = 3/max(distance_between_two_stations);
  real rho = 0.05;
...
transformed parameters {
  ...
  */
  ...
  //  right_y[1:(p+1), 1:S] = rep_matrix(y_start,S);//bisognerebbe togliere cos
  right_y[1:(p+1), 1:S] = rep_matrix(mean(media_stazione),(p+1),S);//bisognerebbe togliere cos
  right_y[(p+2):(T+p+1), 1:S] = y - cos_of_day - regres - spatial; 
...
generated quantities {
  ...
  //  y_post_pred_aux[1:(p+1), 1:S] = rep_matrix(y_start,S); //bisognerebbe aggiungere cos
  y_post_pred_aux[1:(p+1), 1:S] = rep_matrix(mean(media_stazione),(1+p),S); //bisognerebbe aggiungere cos
