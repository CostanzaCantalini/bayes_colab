...
parameters {
  ...
  real hyper_y_start_m;
  real  hyper_y_start_s;
...
transformed parameters {
  ...
  right_y[(p+2):(T+p+1), 1:S] = right_y[(p+2):(T+p+1), 1:S] - cos_of_day;
...
model {
  ...
  for (j in 1:(p+1)) {
    y_start[j,1:S] ~ normal(hyper_y_start_m,hyper_y_start_s);
  }
...
generated quantities {
  ...
  y_post_pred_aux[1:(p+1), 1:S] = y_start;
  ...
  matrix[T,S] y_post_pred = y_post_pred_aux[p+2:T+p+1,:] + cos_of_day[:,:];
