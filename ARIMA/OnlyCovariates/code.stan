...
transformed parameters {
  ...
  right_y[1:(p+1), 1:S] = rep_matrix(y_0,p+1,S);
  right_y[(p+2):(T+p+1), 1:S] = y; 
  for (k in 1:missing_size) {
    right_y[p+1+u_mine[k],v[k]] = w[k];
  }
  right_y[(p+2):(T+p+1), 1:S] = right_y[(p+2):(T+p+1), 1:S] - cos_of_day - regres;
...
model {
  ...
  y_0 ~ normal(mean(media_stazione),1);
...
generated quantities {
  ...
  y_post_pred_aux[1:(p+1), 1:S] = rep_matrix(y_0,p+1,S);
  ...
  matrix[T,S] y_post_pred = y_post_pred_aux[p+2:T+p+1,:] + cos_of_day[:,:] + regres[:,:];
